...
export function FranchPage({ today, detailRows = today }) {
  ...
  const lojas = useMemo(() => {
    const baseMetrics = buildStoreMetrics(today);
    const allScores = baseMetrics.map((store) => store.disponib);
    const mediaRede = Math.round(
      allScores.reduce((s, x) => s + x, 0) / (allScores.length || 1)
    );
    const sorted = baseMetrics
      .map((m) => {
        const score = m.disponib;
        const rank = baseMetrics.filter((x) => x.disponib < score).length + 1;
        return {
          ...m,
          cats: m.cats,
          score,
  ...
  const selItems = sel ? detailRows.filter((r) => r.loja === sel.loja) : [];
          ...
          <Card>
            <div style={{ display: 'flex', gap: 3, background: C.bg, borderRadius: 9, padding: 3, marginBottom: 12 }}>
              {[
                ...
                ['ativos', `Ativos (${sel?.a || 0})`, 'check', C.green],
              ].map(([v, l, ic, c]) => (
                <button
                  key={v}
            ...
            <div style={{ maxHeight: 360, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 5 }}>
              ...
              {(itemTab === 'pausados' ? selPausados : selAtivos).length === 0 && (
                <div style={{ textAlign: 'center', padding: 24, color: C.muted, fontSize: 13 }}>
                  {itemTab === 'pausados'
                    ? '✅ Nenhum item pausado!'
                    : `${sel?.a || 0} itens ativos. O XLSX traz o total, mas não a lista nominal dos ativos.`}
                </div>
              )}
            </div>
          </Card>
